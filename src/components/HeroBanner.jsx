...
const HeroBanner = () => {
  ...
  const titles = ["Catalyzing growth", "Driving innovation", "Think tank", "Trusted partner"];
  ...
  const handlePrevClick = () => {
    ...
    setTitleIndex((prevIndex) => (prevIndex === 0 ? titles.length - 1 : prevIndex - 1)); // Update title index
  };
  
  const handleNextClick = () => {
    swiper.slideNext(); // Move to the next slide
    setColorIndex((prevIndex) => (prevIndex === colors.length - 1 ? 0 : prevIndex + 1)); // Update color index
    setTitleIndex((prevIndex) => (prevIndex === titles.length - 1 ? 0 : prevIndex + 1)); // Update title index
  };
      ...
      >
        ...
        <div className="swiper-nav-btns flex gap-8 items-center w-[40%] justify-center">
          ...
          <button
            ...
            className=" flex items-center text-black hover:text-white gap-8 font-semibold"
          >
            {titleIndex === titles.length - 1 ? titles[0] : titles[titleIndex + 1]} <FaAngleRight />
          </button>
        </div>
      </div>
    </div>
